...
RSAの問題の解法は自分メモとして [RSA](https://www.notion.so/RSA-6b13caf305e0411ca28a3e0fa57168c2?pvs=21) にまとめてある。
...
最初の4文字が英字であることが期待されるので、そのような条件を満たすようなシードをリストアップすることとする。以下の `[seedlist.py](http://seedlist.py)` （試行錯誤ゆえめちゃ汚い）を実行すると、 `seedlist.hex` のファイルにシード候補たちが1行ずつ書かれる。
...
自分メモの [sleuthkit 基本仕草](https://www.notion.so/sleuthkit-d96978e1f5864cf9b7d15a502a6f93cd?pvs=21) あたりを読みつつ sleuthkit でやっていく。
...
[パスワード付きzipのクラック with john](https://www.notion.so/zip-with-john-6096ebfa313b4b56b992cf80e2ecc5ee?pvs=21) でもパスワード特定できず。
...
[Fuzz: Information disclosure / Access control / GraphQL (hidden api) / JWT / OAuth](https://www.notion.so/Fuzz-Information-disclosure-Access-control-GraphQL-hidden-api-JWT-OAuth-eab8dcf67d7f4e8e8920fbb6e96d255f?pvs=21)
...
[[server] JWT attacks](https://www.notion.so/server-JWT-attacks-8fbb303a58be4c769720f3e5cd072861?pvs=21) にまとめたいろいろな攻撃を試すが、問題タイトル的にも本命は [Exploit: 秘密鍵にbrute force / 辞書攻撃](https://www.notion.so/Exploit-brute-force-86d4ae8ce83e4d0cb4e3f80a0e4050a4?pvs=21) 。
